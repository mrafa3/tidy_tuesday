...
df_premodern %>% 
  ...
  geom_text(
    ...
    aes(label = paste("#", annual_visitor_rank, unit_name),
        color = unit_name),
    hjust = 0,
    nudge_x = 1,
    size = 3,
  ...
  labs(x='',
       ...
       subtitle = subtitle,
       caption = caption) + 
  scale_x_continuous(limits = c(1904, 1985)) + 
...
(df_tbl <- df_annual %>% 
  ...
  filter(year == 2016) %>% 
  select(rank_visitors_2016=annual_visitor_rank, unit_name, visitors, state, year_opened) %>% 
  left_join(x=.,
            y=df_maintenance,
            by=c('unit_name' = 'park_name')) %>% 
  ungroup() %>% 
...
visitors_palette <- col_numeric(c("lightgreen", "darkgreen"), 
...
(tbl_viz <- df_tbl %>% 
  gt() %>% 
  #rename columns
  cols_label(rank_visitors_2016 = 'Rank Visitors (2016)',
             unit_name = 'Park',
             visitors = 'Visitors (2016)',
             state = 'State',
...
* Adding decade over decade % change would be neat
